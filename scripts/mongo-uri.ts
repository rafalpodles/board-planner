 ...
 * Both dump-collections and migrate-legacy-fields run from a laptop, so they have
 * to find the reachable one.
 */
 ...
 * driver and Mongoose then quietly fall back to `test`. Production genuinely does
 * live in `test`, so this cannot be validated by name — callers check content.
 */
