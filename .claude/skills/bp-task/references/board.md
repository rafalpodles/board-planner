...
- Assignees are usernames, and the board refuses one that is not a member — so read the name rather than writing it down here. **The board's own account** is the `createdBy.username` `list_tasks` returns, taken from a task **neither `pm` nor `claude` created**: those two are person-shaped accounts that hand the task to nobody, and the PM agent files tasks under `pm`, so reading one of those is how you assign a task to nobody while following this line. (`get_project` carries a `createdBy` too and would be single-valued, but on this board it is `null` — the project predates the field.)
