...
function apiSpy() {
  return {
    claim: vi.fn(),
    setStatus: vi
      .fn<(projectId: string, taskId: string, status: string) => Promise<void>>()
      .mockResolvedValue(undefined),
    comment: vi
      .fn<(projectId: string, taskId: string, body: string) => Promise<void>>()
      .mockResolvedValue(undefined),
...
describe("createReporter", () => {
  it("routes a blocked task to the board's review column with the reason", async () => {
    ...
    expect(api.setStatus).toHaveBeenCalledWith("CP", "t1", "checking");
    expect(api.comment.mock.calls[0][2]).toMatch(/requirements are ambiguous/);
  });
  ...
  it("names the gate and the pushed branch when a gate rejects", async () => {
    ...
    expect(api.setStatus).toHaveBeenCalledWith("CP", "t1", "checking");
    expect(api.comment.mock.calls[0][2]).toMatch(/diff-size/);
    expect(api.comment.mock.calls[0][2]).toMatch(/900 lines/);
  ...
  it("releases through the release endpoint so the attempt is given back", async () => {
    ...
    expect(api.comment.mock.calls[0][2]).toMatch(/usage limit reached/);
  });
  ...
  it("requeues a crashed run through the release endpoint, charging the attempt", async () => {
    ...
    expect(api.setStatus).not.toHaveBeenCalled();
    expect(api.comment.mock.calls[0][2]).toMatch(/the run timed out/);
  });
  ...
  it("says which attempt failed when it requeues, so a board reader can see them climb", async () => {
    ...
    expect(api.comment.mock.calls[0][2]).toMatch(/attempt 2/);
  });
  ...
  it("closes a merged task in the board's done column with the PR url", async () => {
    ...
    expect(api.setStatus).toHaveBeenCalledWith("CP", "t1", "shipped");
    expect(api.comment.mock.calls[0][2]).toMatch(/pull\/7/);
  ...
  it("caps an oversized agent-authored summary", async () => {
    ...
    expect(api.comment.mock.calls[0][2].length).toBeLessThan(3000);
  });
  ...
  it("comments for every task released for the same reason, not just the first", async () => {
    ...
    expect(api.comment.mock.calls[1][1]).toBe("t2");
  });
  ...
  it("names the attempt it gave up on without mangling the grammar", async () => {
    ...
    expect(api.setStatus).toHaveBeenCalledWith("CP", "t1", "checking");
    expect(api.comment.mock.calls[0][2]).toMatch(/attempt 1\b/);
    expect(api.comment.mock.calls[0][2]).not.toMatch(/1 attempts/);
    expect(api.comment.mock.calls[0][2]).toMatch(/merge failed/);
  });
  ...
  it("omits the attempt count when the board did not report one", async () => {
    ...
    expect(api.comment.mock.calls[0][2]).not.toMatch(/attempt 0/);
    expect(api.comment.mock.calls[0][2]).toMatch(/merge failed/);
  });
});
...
describe("reports the server refused", () => {
  ...
  it("queues a merge comment the server would not take, so the task is not stranded", async () => {
    ...
    expect(outbox.add).toHaveBeenCalledWith(
      expect.objectContaining({ kind: "comment", projectId: "CP", taskId: "t1" })
    );
  });
