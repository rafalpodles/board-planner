...
test("with nothing to show, the list says so — and only an admin is offered a first board", async ({
  ...
  await test.step("an account with no grant anywhere", async () => {
    ...
    // Only that there are no boards: an account on none is not told who the admins are
...
// The "not on any board" screen must only appear when the read actually said there are no boards
