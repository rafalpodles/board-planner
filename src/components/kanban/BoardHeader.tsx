...
import { ALL_TASKS, BACKLOG, sprintScopeLabel } from "@/lib/sprint-scope";
...
export function BoardHeader({
  ...
  // On a phone this header does not get a row of its own — it is rendered into the app bar, which
  // otherwise sat above it doing nothing but naming the app. The slot only exists below `md`, so
  // on a wide screen this stays null and the header renders where it always did.
  // Same query the board's paging uses, and the same one the sidebar uses for `isMobile`
  const onPhone = useMediaQuery("(max-width: 767px)");
  const [slot, setSlot] = useState<HTMLElement | null>(null);
  useEffect(() => {
    const find = () => setSlot(document.getElementById("mobile-header-slot"));
    find();
    // The bar is `md:hidden`, so the slot leaves and re-enters the DOM as the window crosses it
    const media = window.matchMedia("(max-width: 767px)");
    media.addEventListener("change", find);
    return () => media.removeEventListener("change", find);
  }, []);
  ...
  const compact =
    onPhone && slot
      ? createPortal(
          <>
            <h1 className="min-w-0 flex-1 truncate text-sm font-bold leading-tight">
              {projectName}
            </h1>
            <div className="flex shrink-0 items-center rounded-lg border border-border bg-bg-card p-0.5">
              {(["board", "list"] as const).map((mode) => (
                <button
                  ...
                  className={`focus-ring rounded-md px-1.5 py-1 text-[11px] transition-colors ${
                    viewMode === mode
  ...
  // On a phone the header has no row of its own: everything a reader needs from it is in the app
  // bar above, and the rest — the description, the scope menu, refresh — is not what a phone is for.
  if (onPhone) return compact;
      ...
      <header className="@container relative z-30 flex h-14 shrink-0 items-center gap-2 @md:gap-3 border-b border-border bg-bg px-3 @md:px-6">
        <div className="min-w-0">
          ...
          <div className="flex items-center gap-2">
            <span
              aria-hidden
              className="hidden shrink-0 text-2xl leading-none @md:inline"
            >
        ...
        >
          ...
          <kbd className="ml-1 hidden rounded bg-black/25 px-1 text-[11px] @xl:inline">
            N
          </kbd>
        </Button>
      </header>
